<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&height=240&color=gradient&customColorList=6,12,20,24,30&text=Rohith%20Goturi&fontSize=55&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Software%20Engineer%20%7C%20AI%20Engineer%20%7C%20Full%20Stack%20Developer&descAlignY=60"/>

<br>

<img src="https://readme-typing-svg.herokuapp.com?font=Poppins&weight=600&size=26&pause=1000&color=8A2BE2&center=true&vCenter=true&random=false&width=900&lines=Software+Engineer;AI+Engineer;Full+Stack+Developer;Java+%7C+Spring+Boot+%7C+React;Building+AI-Powered+Applications;Open+Source+Enthusiast;Always+Learning+New+Technologies" />

<br><br>

![](https://img.shields.io/badge/B.Tech-CSE-blueviolet?style=for-the-badge)
![](https://img.shields.io/badge/Open%20to-Internships-blue?style=for-the-badge)

<br>

<a href="https://github.com/rohithgoturi">
<img src="https://img.shields.io/badge/GitHub-rohithgoturi-181717?style=for-the-badge&logo=github"/>
</a>

<a href="mailto:rohithgoturi28@gmail.com">
<img src="https://img.shields.io/badge/Gmail-Contact-EA4335?style=for-the-badge&logo=gmail"/>
</a>

<a href="https://linkedin.com/in/rohith-goturi">
<img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin"/>
</a>

<a href="https://your-portfolio.vercel.app">
<img src="https://img.shields.io/badge/Portfolio-Visit-8A2BE2?style=for-the-badge&logo=vercel"/>
</a>

<br><br>

![](https://komarev.com/ghpvc/?username=rohithgoturi&style=for-the-badge&color=blueviolet)

![](https://img.shields.io/github/followers/rohithgoturi?style=for-the-badge&color=8A2BE2)

![](https://img.shields.io/github/stars/rohithgoturi?affiliations=OWNER&style=for-the-badge&color=6f42c1)

</div>

---

# About Me

I'm a passionate **Software Engineer** and **AI Engineer** focused on building intelligent, scalable, and production-ready software.

My primary interests lie in **Software Engineering**, **Artificial Intelligence**, **LLMs**, **Agentic AI**, **Backend Engineering**, and **Full Stack Development**. I enjoy solving real-world problems through modern technologies while following clean architecture, scalable design patterns, and engineering best practices.

Currently, I'm pursuing my **B.Tech in Computer Science & Engineering** while continuously improving my Data Structures & Algorithms, System Design fundamentals, and software development skills.

I believe great software is built by combining clean code, problem-solving ability, and user-centric thinking.

### Open To

- Software Engineering Internships
- AI Engineering Roles
- Full Stack Development
- Open Source Contributions
- Hackathons
- Research Collaborations

---

# Tech Stack

## Languages

<p>

<img src="https://skillicons.dev/icons?i=java,python,javascript,c"/>

</p>

## Frontend

<p>

<img src="https://skillicons.dev/icons?i=html,css,react,tailwind,bootstrap"/>

</p>

## Backend & Databases

<p>

<img src="https://skillicons.dev/icons?i=spring,nodejs,express,postgresql,mysql,mongodb"/>

</p>

## AI / ML

<p>

<img src="https://skillicons.dev/icons?i=python,tensorflow,pytorch"/>

</p>

## Cloud • DevOps • Tooling

<p>

<img src="https://skillicons.dev/icons?i=git,github,docker,vscode,postman,linux"/>

</p>

---

# Featured Projects

<details close>
<summary><b>🚀 AI Career Copilot</b></summary>

<br>

An AI-powered career assistant that helps students and job seekers improve their resumes, analyze skill gaps, match job descriptions, and receive personalized career guidance using Large Language Models.

| Attribute | Details |
|------------|---------|
| **Role** | Full Stack Developer |
| **Stack** | React • Node.js • Express • MongoDB • Tailwind CSS • LLM |
| **Scale** | Resume Analysis • Job Matching • Skill Gap Detection |
| **Performance** | Optimized REST APIs for efficient document processing |
| **Security** | Secure API architecture and protected user data |
| **Impact** | Enables users to understand missing skills and receive AI-driven career recommendations |
| **Repository** | https://github.com/rohithgoturi |

### Key Highlights

- AI-powered resume parsing
- Job description matching
- Personalized skill-gap analysis
- Intelligent career recommendations using LLMs
- Optimized backend APIs for fast document processing
- Clean and scalable full-stack architecture

</details>

---

<details close>
<summary><b>🎙️ AI-Powered Multi-Agent Voice Lead Qualification Platform</b></summary>

<br>

A production-oriented AI platform that automates lead qualification through multiple AI agents capable of analyzing conversations, scoring leads, summarizing calls, and assisting sales teams.

| Attribute | Details |
|------------|---------|
| **Role** | Full Stack Developer |
| **Stack** | React • Tailwind CSS • Spring Boot • PostgreSQL • Supabase |
| **Scale** | Multi-Agent AI Workflow |
| **Performance** | Fast REST APIs with scalable backend architecture |
| **Security** | Secure authentication and role-based access |
| **Impact** | Automates lead qualification and improves sales efficiency |
| **Repository** | https://github.com/rohithgoturi |

### Key Highlights

- Multi-agent AI architecture
- AI-powered lead scoring
- Conversation intelligence
- Automatic call summarization
- Admin dashboard
- Secure authentication
- Scalable Spring Boot backend

</details>

---

<details close>
<summary><b>🛡️ AI-Powered Deepfake & Morphed Content Detector</b></summary>

<br>

An AI-powered digital identity protection platform designed to detect deepfake images and morphed media using computer vision and deep learning techniques. The system helps verify digital content authenticity by analyzing visual artifacts and AI-generated manipulations.

| Attribute | Details |
|------------|---------|
| **Role** | Full Stack Developer |
| **Stack** | React • Spring Boot • Python • PostgreSQL • OpenCV • Deep Learning |
| **Scale** | AI-Based Image Verification Platform |
| **Performance** | Optimized image analysis pipeline with fast prediction and REST API integration |
| **Security** | Secure file uploads, authenticated access, and protected prediction APIs |
| **Impact** | Helps identify manipulated media, improving digital trust and preventing identity fraud |
| **Repository** | https://github.com/rohithgoturi |

### Key Highlights

- Detects AI-generated deepfake and morphed images
- Computer vision pipeline for image authenticity analysis
- Deep learning model integration for prediction
- Secure image upload and processing workflow
- REST APIs for inference and result management
- Interactive dashboard with prediction reports
- Scalable full-stack architecture using React and Spring Boot

</details>

---

# Experience

## Student Independent Developer

**Self-Employed**  
**Aug 2025 – Present**  
**Nagpur, India**

Developing production-ready AI-powered software solutions by combining Full Stack Development, Artificial Intelligence, and Software Engineering best practices. Building projects focused on automation, intelligent recommendations, and scalable backend systems while actively participating in hackathons and open-source development. :contentReference[oaicite:0]{index=0}

### Responsibilities

- Engineered **3 AI-powered full-stack applications** using React, Spring Boot, PostgreSQL, Java, and Python.
- Developed **15+ REST API endpoints** with secure authentication and optimized database design.
- Built **LLM-powered and Agentic AI workflows** for automation and intelligent decision-making.
- Applied **Clean Architecture**, Git workflows, testing methodologies, and scalable software engineering principles.
- Collaborated on hackathons and continuously explored modern AI technologies.

### Technologies

`Java` • `React` • `Spring Boot` • `Node.js` • `Express` • `PostgreSQL` • `MongoDB` • `Python` • `REST APIs` • `Git` • `GitHub` • `Tailwind CSS`

---
# 📊 GitHub Stats:
![](https://github-readme-stats.shion.dev/api?username=rohithgoturi&theme=dark&hide_border=false&include_all_commits=true&count_private=true)<br/>
![](https://streak-stats.demolab.com/?user=rohithgoturi&theme=dark&hide_border=false)<br/>
![](https://github-readme-stats.shion.dev/api/top-langs/?username=rohithgoturi&theme=dark&hide_border=false&include_all_commits=true&count_private=true&layout=compact)

---
[![](https://komarev.com/ghpvc/?username=rohithgoturi&icon=0&color=0)](https://visitcount.itsvg.in)

<!-- Proudly created with GPRM ( https://gprm.itsvg.in ) -->
---

# Contribution Activity

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=rohithgoturi&theme=tokyo-night&hide_border=true&area=true"/>

</div>

---

# Contribution Snake

<div align="center">

<p><i>Automatically generated using GitHub Actions.</i></p>

<img src="https://raw.githubusercontent.com/rohithgoturi/rohithgoturi/output/github-contribution-grid-snake-dark.svg"/>

</div>

> **Note:** To make the snake animation work, you'll need to set up the GitHub Action in your profile repository after publishing your README.

---

# Connect With Me

<div align="center">

<a href="mailto:rohithgoturi28@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://github.com/rohithgoturi">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://linkedin.com/in/rohith-goturi">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://your-portfolio.vercel.app">
<img src="https://img.shields.io/badge/Portfolio-8A2BE2?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

</div>

---

# Support My Work

<div align="center">

If you enjoy my projects or find them helpful, consider giving them a ⭐ on GitHub.

I enjoy collaborating on AI, Full Stack Development, Open Source, and Software Engineering projects.

</div>

---

<div align="center">

## "Building scalable software today to solve tomorrow's challenges."

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&height=120&section=footer&color=gradient&customColorList=6,12,20,24,30"/>

</div>
